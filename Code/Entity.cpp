...
Entity::~Entity()
{
	...
	if (cameraBufferCPU != nullptr)
		delete cameraBufferCPU;
}
...
// Function is similar to color, but has extra uniforms.
void Entity::CreateDescriptorSetRay()
{
	Demo* demo = Demo::GetInstance();
	...
	// We only need camera information for a ray marched entity, as our vertex
	// shader is going to output a large triangle only with the fragment
	// shader doing the rest.
	VkBufferCreateInfo buf_info = {};
	buf_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
	buf_info.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
	buf_info.size = sizeof(camera_struct);

	// we make a new CPU buffer, and we copy our data into the buffer.
	// we give memory_properties (which was created earlier) to help us
	// create the buffer
	cameraBufferCPU = new BufferCPU(demo->device, demo->memory_properties, buf_info);
	...
	alloc_info.descriptorPool = demo->desc_pool;
	alloc_info.descriptorSetCount = 1;
	alloc_info.pSetLayouts = &demo->desc_layout_ray;
	vkAllocateDescriptorSets(demo->device, &alloc_info, &descriptor_set);

	// The first descriptor will be the uniform buffer
	// because this descriptor is at binding #0 of the shader
	VkDescriptorBufferInfo buffer_info = {};
	buffer_info.range = sizeof(camera_struct);
	buffer_info.buffer = cameraBufferCPU->buffer;
...
void Entity::Draw(VkCommandBuffer cmd, VkPipelineLayout pipeline_layout)
{
	...
	// We are doing ray marching.
	if (cameraBufferCPU != nullptr) {
		// We draw a simple triangle into the vertex shader, which
		// has hardcoded vertices to cover the entire screen. This
		// allows us to shoot rays to every pixel on the screen.
...
void Entity::Update()
{
	...
	{
		if (camera == nullptr) {
			std::printf("ERROR: Ray marching enabled without passing in the camera! In Scene.cpp, set the pointer in the entity to the camera!\n");
		}

		// we are only storing camera position.
		camera_struct temporaryData;

		temporaryData.cameraPos = camera->GetPosition();
		temporaryData.cameraDir = camera->GetForward();
		temporaryData.aspectRatio = (float)demo->width/(float)demo->height;
		temporaryData.time = (*totalRunTime);

		cameraBufferCPU->Store(&temporaryData, sizeof(camera_struct));
	}
}
