...
Scene::Scene()
{
	...
	// Create the camera.
	camera = new Camera(glm::vec3(5.0, 0.0, 0.0), &d->window);
	...
	rayRect = new RayEntity(camera, &totalRunTime);


	// Create the enities for the scene.
	//sphereMesh = new Mesh("../../../Assets/sphere.obj", false);
	//
	//// Make the spheres.
	//std::shared_ptr<Entity> sphere = std::make_shared<Entity>();
	//sphere->mesh = sphereMesh;
	//sphere->pos = glm::vec3(-0.25f, 0.0f, 0.0f);
	//// Since we are using ray tracing, we have to pass in camera pos.
	//sphere->cameraPos = &cameraPosition;
	//sphere->CreateDescriptorSetRay();
	//entities.push_back(sphere);
	//
	//sphere = std::make_shared<Entity>();
	//sphere->mesh = sphereMesh;
	//sphere->pos = glm::vec3(03.25f, 0.0f, 0.0f);
	//// Since we are using ray tracing, we have to pass in camera pos.
	//sphere->cameraPos = &cameraPosition;
	//sphere->CreateDescriptorSetRay();
	//entities.push_back(sphere);
...
void Scene::CheckKeyboardInput() {
	...
	if (keys['D']) {
		camera->MoveLocal(glm::vec3(0.1f, 0.0f, 0.0f));
	}
	if (keys['A'])
	{
		camera->MoveLocal(glm::vec3(-0.1f, 0.0f, 0.0f));
	...
	if (keys['W'])
	{
		camera->MoveLocal(glm::vec3(0.0f, 0.0f, 0.1f));
	}
	if (keys['S']) {
		camera->MoveLocal(glm::vec3(0.0f, 0.0f, -0.1f));
	}
	...
	if (keys[VK_SPACE])
	{
		camera->MoveLocal(glm::vec3(0.0f, 0.1f, 0.0f));
	}
	if (keys[VK_SHIFT]) {
		camera->MoveLocal(glm::vec3(0.0f, -0.1f, 0.0f));
	}
...
void Scene::UpdateCamera() {
	...
	d->view_matrix = camera->GetViewMatrix();	// build the view matrix from thsi data
	...
	skyEntity->pos = camera->GetPosition();
