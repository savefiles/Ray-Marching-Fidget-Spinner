...
// This pipeline will be most similar to the 2d pipeline.
// We are not rendering models, we are using exclusively 
// sdf to generate our objects. We render two triangles
// that cover the entire screen so that the fragment shader
// triggers.

PipeRay::PipeRay(VkDevice dev, VkDescriptorSetLayout desc_layout, VkRenderPass render_pass)
{
	device = dev;
